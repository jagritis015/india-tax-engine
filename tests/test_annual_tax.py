...
def test_new_regime_12_lakh_has_zero_tax_after_rebate():
    result = calculate_annual_tax(Decimal("1200000"), "2026-27", TaxRegime.NEW)
...
def test_new_regime_12_10_lakh_marginal_relief_and_cess():
    result = calculate_annual_tax(Decimal("1210000"), "2026-27", TaxRegime.NEW)
...
def test_new_regime_15_lakh():
    result = calculate_annual_tax(Decimal("1500000"), "2026-27", TaxRegime.NEW)
...
def test_old_regime_15_lakh():
    result = calculate_annual_tax(Decimal("1500000"), "2026-27", TaxRegime.OLD)
...
def test_old_regime_5_lakh_gets_section_156_rebate():
    result = calculate_annual_tax(Decimal("500000"), "2026-27", TaxRegime.OLD, resident_individual=True)
...
def test_old_regime_above_5_lakh_gets_no_rebate():
    result = calculate_annual_tax(Decimal("501000"), "2026-27", TaxRegime.OLD, resident_individual=True)
    assert result["rebate"] == Decimal("0")
    assert result["annual_tax_liability"] > Decimal("0")


def test_non_resident_does_not_get_old_regime_rebate():
    result = calculate_annual_tax(Decimal("500000"), "2026-27", TaxRegime.OLD, resident_individual=False)
    assert result["rebate"] == Decimal("0")


def test_annual_tax_fails_closed_for_unregistered_tax_year():
    with pytest.raises(StatutoryRuleUnavailableError):
        calculate_annual_tax(Decimal("1200000"), "2027-28", TaxRegime.NEW)
