...
type Calculation = { status?: string; error?: string; inputComponents?: Array<{code:string;amount:number}>; calculatedComponents?: Array<{code:string;amount:number|null}>; result?: {status:string;grossSalary:number;taxableIncome:number;incomeTaxBeforeSurcharge:number;surchargeRatePercent:number;surchargeBeforeRelief:number;surchargeMarginalRelief:number;surcharge:number;cess:number;annualTaxLiability:number;tds:number;employeePf:number;professionalTax:number;totalDeductions:number;netSalary:number;reviewReason?:string} };
    ...
    <section style={{...panel,marginTop:18,padding:0,overflow:"hidden"}}>
      ...
      <div style={{padding:20,display:"grid",gap:18}}>
        ...
        {result?.result?.status==="CALCULATED"&&<section style={resultPanel}>
          ...
          <details style={explain}><summary style={{cursor:"pointer",fontWeight:800,color:"#204d40"}}>How this TDS was calculated</summary><div style={{marginTop:13,display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(170px,1fr))",gap:10}}><AuditLine label="Projected annual salary" value={money.format(Number(payrollContext.taxableSalaryYtd)+(result.result.grossSalary*7))}/><AuditLine label="Taxable income" value={money.format(result.result.taxableIncome)}/><AuditLine label="Income tax before surcharge" value={money.format(result.result.incomeTaxBeforeSurcharge)}/><AuditLine label={`Surcharge at ${result.result.surchargeRatePercent}%`} value={money.format(result.result.surcharge)}/><AuditLine label="Surcharge before relief" value={money.format(result.result.surchargeBeforeRelief)}/><AuditLine label="Marginal relief" value={money.format(result.result.surchargeMarginalRelief)}/><AuditLine label="Health and Education Cess" value={money.format(result.result.cess)}/><AuditLine label="Annual tax liability" value={money.format(result.result.annualTaxLiability)}/><AuditLine label="TDS already deducted" value={money.format(Number(payrollContext.tdsDeductedYtd))}/></div><p style={{margin:"13px 0 0",fontSize:13,color:"#59675f",lineHeight:1.55}}>The deterministic engine applies the new regime slabs, then salary surcharge at 10%, 15%, or 25% with marginal relief at the statutory thresholds. It applies 4% cess after surcharge, subtracts TDS already deducted, and spreads the balance over September to March. Special-rate income remains outside this UAT path.</p></details>
        </section>}
