  ...
  return <main style={{minHeight:"100vh",background:"#f6f8fa",fontFamily:"Inter,ui-sans-serif,system-ui,sans-serif",color:"#18212b"}}>
    <div style={{maxWidth:1120,margin:"0 auto",padding:"28px 18px 64px"}}>
      ...
      <section style={{background:"white",border:"1px solid #dde3e8",borderRadius:12,overflow:"hidden"}}>
        <div style={{padding:18,borderBottom:"1px solid #e7ebee"}}><h2 style={{margin:0}}>Evidence ledger</h2><p style={{margin:"6px 0 0",color:"#68727c"}}>Each U.S. day keeps its source, classification and verification status.</p></div>
        <div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",minWidth:850}}><thead><tr>{["Date","Country","Classification","Evidence","Status","Note"].map(x=><th key={x} style={{textAlign:"left",fontSize:12,color:"#6f7983",padding:"11px 14px",background:"#fafbfc",borderBottom:"1px solid #e7ebee"}}>{x}</th>)}</tr></thead><tbody>{ADITI_DAY_LEDGER.map(item=><tr key={item.id}><td style={cell}>{item.date}</td><td style={cell}>{item.country}</td><td style={cell}>{item.classification}</td><td style={cell}>{item.evidence}</td><td style={cell}><strong>{item.evidenceStatus}</strong></td><td style={cell}>{item.note}</td></tr>)}</tbody></table></div>
      </section>
