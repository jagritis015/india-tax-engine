...
export default function Home(){
  ...
  return <main className="product-shell">
    <aside className={`sidebar ${menu?"mobile-open":""}`}>
      ...
      <nav>{nav.map(({id,label,icon:Icon})=>id==="mobility"?<a key={id} className="nav-item" href="/uat/mobility"><Icon/>{label}</a>:<button key={id} className={`nav-item ${view===id?"active":""}`} onClick={()=>go(id)}><Icon/>{label}</button>)}</nav>
...
function Scenarios(){const packs=[{name:"Monthly baseline",records:"100 employees",copy:"Regular September payroll with unresolved source records kept visible."},{name:"Variable pay",records:"100 employees",copy:"Festival bonus and sales commission changes across the full company."},{name:"Statutory boundaries",records:"7 controls",copy:"Karnataka PT, PF ceiling, rebate and marginal relief thresholds."},{name:"Immigration controls",records:"8 cases",copy:"Complete, missing, expired and date-gap evidence files."}];return <><Title title="Real-use Scenario Lab" copy="Run realistic payroll and mobility cases through the live services and reconcile actual results with downloadable expected outputs." action={<a className="primary-action" style={{textDecoration:"none"}} href="/uat/scenarios">Open Scenario Lab <ChevronRight/></a>}/><div className="uat-notice"><FlaskConical/><span><strong>Executable UAT pack</strong>100 employee master, 221 payroll records, eight immigration cases and a step-by-step test script.</span></div><div className="compliance-grid">{packs.map(pack=><article className="compliance-card" key={pack.name}><div><FlaskConical/><span className="employee-status ready">Runnable</span></div><h2>{pack.name}</h2><p>{pack.copy}</p><small>{pack.records}</small></article>)}</div><div className="security-note"><Download/><span><strong>Download the complete Excel pack</strong><small>Employee master, exact inputs, expected outputs, scenario data and test instructions.</small></span><a className="secondary-action" style={{textDecoration:"none",marginLeft:"auto"}} href="/uat-scenarios/india-payroll-os-real-use-uat-pack.xlsx" download>Download Excel</a></div></>}
function Mobility(){return <><Title title="Global mobility" copy="Open the program-level mobility case queue." action={<a className="primary-action" style={{textDecoration:"none"}} href="/uat/mobility">Open mobility case queue <ChevronRight/></a>}/></>}
