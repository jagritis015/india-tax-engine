...
export default function Home(){
  ...
  const resolve=()=>{if(!selected)return;setIssues(xs=>xs.map(x=>x.id===selected.id?{...x,resolved:true}:x));setSelected(null);notify("Exception resolved in this UAT session.")};
  ...
  const ask=(label:string)=>{const a:Record<string,string>={
    "Explain review items":"NVL-017 and NVL-063 need payroll-input review. NVL-042 requires Professional Tax work-location review before approval.",
    "Show blocking issues":blockers?"NVL-042, Pooja Sharma, is the current blocker because the Professional Tax work location still requires review.":"No blocking issues remain. Review any remaining non-blocking items before approval.",
  ...
  return <main className="product-shell">
    ...
    <section className="workspace">
      ...
      <div className="content">
        {view==="overview"&&<Overview issues={issues} open={open} status={status} go={go} select={setSelected} assistant={assistant} setAssistant={setAssistant} answer={answer} ask={ask} employeeCount={people.length}/>}
        {view==="payroll"&&<Payroll issues={issues} open={open} status={status} select={setSelected} submit={submit} employeeCount={people.length}/>}
    ...
    {selected&&<div className="modal-backdrop" onMouseDown={()=>setSelected(null)}><section className="modal" role="dialog" aria-modal="true" onMouseDown={e=>e.stopPropagation()}><div className="modal-head"><div><span className="section-label">Payroll exception</span><h2>{selected.employee}</h2><small>{selected.employeeId}</small></div><button className="icon-button" onClick={()=>setSelected(null)}><X/></button></div><span className={`issue-pill ${selected.severity.toLowerCase()}`}>{selected.severity}</span><h3>{selected.issue}</h3><p>{selected.severity==="Blocker"?"This item must be reviewed before the UAT payroll can move to approval. No statutory result is inferred while the required work-location evidence is unresolved.":"Review the representative payroll input and confirm the UAT exception only after the source data is acceptable."}</p><div className="modal-actions"><button className="secondary-action" onClick={()=>setSelected(null)}>Cancel</button><button className="primary-action" onClick={resolve}><Check/>Confirm and resolve</button></div></section></div>}
