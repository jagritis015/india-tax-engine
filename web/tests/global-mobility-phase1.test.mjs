...
test("stale obligation edit is rejected without a result",async()=>{
  ...
  assert.match(outcome.error,/stale edit/i); assert.equal("result" in outcome,false); assert.equal(current.status,"OPEN");
});
