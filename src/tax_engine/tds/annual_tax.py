...
def _assert_annual_tax_rules_verified(tax_year: str, regime: TaxRegime, apply_surcharge: bool) -> None:
    """Fail closed before statutory arithmetic executes."""
    on_date = _tax_year_start(tax_year)
    required = [
        "ROUNDING",
        "NEW_REGIME_RATES" if regime == TaxRegime.NEW else "OLD_REGIME_RATES",
        "REBATE",
        "HEALTH_EDUCATION_CESS",
    ]
    if apply_surcharge:
        required.append("SURCHARGE")
...
def calculate_tax_from_slabs(taxable_income: Decimal, slabs) -> Decimal:
    ...
    for lower, upper, rate in slabs:
        ...
        if upper is not None:
            amount_in_slab = min(amount_in_slab, upper - lower)
        tax += amount_in_slab * rate
    return tax


def _threshold_for_surcharge(taxable_income: Decimal, regime: TaxRegime) -> Decimal | None:
    if regime == TaxRegime.OLD and taxable_income > Decimal("50000000"):
